...
const SprintPage = () => {
  ...
  const isSprintOver = (() => {
    if (!sprintData?.sprint) return false;
    return new Date(sprintData.sprint.endingDate) < new Date();
  })();
              ...
              <div key={status} className="kanban-column" onDragOver={handleDragOver} onDrop={(e) => handleDrop(e, status)}>
                ...
                <div className="story-list">
                  {getStoriesByStatus(status).map(story => {
                    ...
                    const canMarkRealized = status === 'testing' && isProductOwner && isSprintActive && !story.realized;
                    const canRejectStory = status === 'testing' && isProductOwner && isSprintActive && !story.realized;
